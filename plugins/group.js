...
// Returns true if targetJid is one of the protected owner numbers
function isOwnerTarget(targetJid) {
  const targetNum = (targetJid || '').split('@')[0].split(':')[0].replace(/\D/g, '');
  const protected_ = [
    config.OWNER_NUMBER,
    config.HARD_CODED_OWNER,
  ].filter(Boolean).map(n => String(n).replace(/\D/g, ''));
  return protected_.includes(targetNum);
}
...
cast({
  ...
  try {
    ...
    if (isOwnerTarget(target)) return reply("🛡️ I can't kick my owner!");
...
cast({
  ...
  try {
    ...
    if (isOwnerTarget(target)) return reply("🛡️ My owner is already above everyone — no need to promote!");
...
cast({
  ...
  try {
    ...
    if (isOwnerTarget(target)) return reply("🛡️ I can't demote my owner!");
